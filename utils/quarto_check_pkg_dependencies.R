...
# have changed since the last time the cache was generated. It tracks
# the previous package versions for a quarto render in a CSV file.
#
...
# Echo the differences in packages to the console
if (nrow(diff) > 0) {
  message("The following packages have changed since last quarto render:")
  diff |>
    select(package, previous = hash.x, current = hash.y) |>
    distinct() |>
    arrange(package) |>
    print()
} else{
  message("No package changes detected since last quarto render.")
}
